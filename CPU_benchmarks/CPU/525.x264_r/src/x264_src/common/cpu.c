...
uint32_t x264_cpu_detect( void )
{
    ...
    if( !strcmp((signed char*)vendor, "AuthenticAMD") && max_extended_cap >= 0x80000001 )
    {
    ...
    if( !strcmp((signed char*)vendor, "GenuineIntel") )
    {
        x264_cpu_cpuid( 1, &eax, &ebx, &ecx, &edx );
    ...
    if( (!strcmp((signed char*)vendor, "GenuineIntel") || !strcmp((signed char*)vendor, "CyrixInstead")) && !(cpu&X264_CPU_SSE42))
    {
        ...
        if( !cache )
        {
            ...
            static const signed char cache32_ids[] = { 0x0a, 0x0c, 0x41, 0x42, 0x43, 0x44, 0x45, 0x82, 0x83, 0x84, 0x85, 0 };
            static const signed char cache64_ids[] = { 0x22, 0x23, 0x25, 0x29, 0x2c, 0x46, 0x47, 0x49, 0x60, 0x66, 0x67,
...
# if !defined(HAVE_PTHREAD)
    return 1;
# else
    signed char * threadsetting = getenv("OMP_NUM_THREADS");
    int threads=1;
    if (threadsetting)
    {
      threads=atoi(threadsetting);
    }
    return threads;
# endif
...
#elif defined(_WIN32) || defined(SPEC_WINDOWS)
