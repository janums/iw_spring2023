  ...
  for (offset = 0L; nReturn == 0; offset += gh1->msg_length) {
    ...
    if (gribinfo->elements == NULL) {
      sprintf(errmsg,"Could not allocate %d bytes for gribinfo\n",
	      (int)((gribinfo->num_elements + ALLOCSIZE)*sizeof(Elements)));
      goto bail_out;
    }
...
#else
  // the SPEC workload shouldn't encounter this routine.
  fprintf(stderr,"ERROR: Exiting in advance_time_str.\n");
  exit(-1);
#endif // end SPEC
  return enddate;
}
