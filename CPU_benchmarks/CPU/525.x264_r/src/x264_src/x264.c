...
static struct option long_options[] =
{
    ...
    { "dumpyuv",     required_argument, NULL, OPT_DUMPYUV },
                         ...
                         hnd_t *p_handle, video_info_t *info, cli_input_opt_t *opt )
{
    ...
    int b_auto = !STRCMP( demuxer, "auto" );
    if( !b_regular && b_auto )
        ext = "yuv";
    if( b_regular )
    {
    ...
    if( !STRCMP( module, "avs" ) || !STRCMP( ext, "d2v" ) || !STRCMP( ext, "dga" ) )
    {
#ifdef AVS_INPUT
        input = avs_input;
        module = "avs";
    ...
    else if( !STRCMP( module, "y4m" ) )
        input = y4m_input;
    else if( !STRCMP( module, "yuv" ) )
        input = yuv_input;
    else
    {
#ifdef FFMS_INPUT
        if( b_regular && (b_auto || !STRCMP( demuxer, "ffms" )) &&
...
#endif
#ifdef AVS_INPUT
        if( b_regular && (b_auto || !STRCMP( demuxer, "avs" )) &&
...
#ifdef SPEC
            case OPT_DUMPYUV:
                tga_dump_rate = X264_MAX( atoi( optarg ), 1 );
                break;
#endif
            case OPT_FRAMES:
